...
const EventDetail = () => {
  ...
  const handleRegister = async () => {
    ...
    // For merchandise, validate variant selection
    if (event.eventType === 'merchandise' && event.merchandiseDetails?.variants?.length > 0) {
      ...
      if (sizes.length > 0 && !merchandiseOrder.selectedSize) {
        setError('Please select a size');
        return;
      }
      if (colors.length > 0 && !merchandiseOrder.selectedColor) {
